...
//- (id)initWithFrame:(CGRect)frame withModel:(MyOrderInfoModel *)model {
//    self = [super self];
//    if (self) {
//        self.model = model;
...
- (void)createViewWithModel:(MyOrderInfoModel *)model {
    ...
    //头像
//    if (![self.headImageView isEqual:[NSNull null]]) {
...
//    }
    ...
    self.hosNameLabel.font = [UIFont systemFontOfSize:12];
    ...
    //    [self.baseView addSubview:self.rankLabel];
    //当前状态label
    ...
    //疾病
    self.illLabel.text = @"疾病：";
    self.illTextLabel.text = self.model.ill;
    
    //种类
    self.categoryLabel.text = @"种类：";
    self.categoryTextLabel.text = self.model.category;
    
    //方式
    self.treatLabel.text = @"方式：";
    self.treatTextLabel.text = self.model.treat;
    
    //支付方式
    self.payStyleLabel.text = @"支付方式：";
    self.payStyleTextLbel.text = self.model.pay_method;
    
    //劵
    self.juanLable.text = @"劵：";
    self.juanTextLabel.text = @"请选择";
    ...
    //医保
    self.ybMoneyLabel.text = @"医保：";
    
    //主诉
    self.mainSayLabel.text = @"主诉：";
    self.mainSayTextLabel.text = self.model.main_say;
    
    //现病史
    self.nowDiseaseLabel.text = @"现病史：";
    ...
    //button
    [self.cancelBtn setBackgroundColor:[UIColor stringTOColor:@"#00b09f"]];
    [self.cancelBtn setTitle:@"取消治疗" forState:normal];
    
    [self.sureBtn setBackgroundColor:[UIColor stringTOColor:@"#00b09f"]];
    ...
    if (self.model.type_id == 1 || self.model.type_id == 1.2) {
        ...
        if (self.model.type_id == 1) {//1：等待就诊
            ...
            [self.cancelBtn setTitle:@"取消订单" forState:normal];
            ...
            [self.cancelBtn mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.top.equalTo(self.prepaidLabel.mas_bottom).with.offset(10);
            ...
            [self.checkLabel mas_makeConstraints:^(MASConstraintMaker *make) {
                make.left.equalTo(self.baseView).with.offset(20);
                make.top.equalTo(self.checkDetailLabel.mas_bottom).with.offset(9);
            }];
    ...
    if (self.model.type_id == 3 || self.model.type_id == 1.1) {
        ...
        if (self.model.type_id == 3) {//治疗待支付
            ...
            [self.sureBtn mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.top.equalTo(self.prepaidTextLabel.mas_bottom).with.offset(13);
                make.size.mas_offset(CGSizeMake(100, 30));
            }];
            [self.cancelBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            [self.bgView mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.left.equalTo(self.baseView);
                make.right.equalTo(self.baseView);
                make.bottom.equalTo(self.checkDetailLabel.mas_bottom).with.offset(10);
            }];
    ...
    if (self.model.type_id == 4 || self.model.type_id == 1.3) {
        ...
        if (self.model.type_id == 4) {//治疗完成
            ...
            [self.cancelBtn mas_makeConstraints:^(MASConstraintMaker *make) {
                make.right.equalTo(_baseView.mas_right).with.offset(-20);
                make.top.equalTo(self.bgView.mas_bottom).with.offset(13);
    ...
    if (self.model.type_id == 5 || self.model.type_id == 5.1) {
        ...
        [self.sureBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(_baseView.mas_right).with.offset(-20);
            make.top.equalTo(self.havePayTextLabel.mas_bottom).with.offset(13);
            make.size.mas_offset(CGSizeMake(100, 30));
        }];
        ...
        [self.baseView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.equalTo(self);
            make.right.equalTo(self);
            make.bottom.equalTo(self.sureBtn.mas_bottom);
        }];
...
- (void)allUse {
    ...
    [self.rankLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(self.hosNameLabel.mas_centerY);
        make.left.equalTo(self.hosNameLabel.mas_right).with.offset(12);
    }];
